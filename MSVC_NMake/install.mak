...
install: all
	...
	@if not exist $(PREFIX)\lib\atkmm-$(ATKMM_API_VERSION)\proc\m4\ md $(PREFIX)\lib\atkmm-$(ATKMM_API_VERSION)\proc\m4
	...
	@for %h in ($(ATKMM_BUILD_PRIVATE_HEADERS)) do @if exist ..\atk\atkmm\private\%h copy ..\atk\atkmm\private\%h "$(PREFIX)\include\atkmm-$(ATKMM_API_VERSION)\atkmm\private\%h"
	@for %h in ($(ATKMM_BUILD_PRIVATE_HEADERS)) do @if exist $(OUTDIR)\atkmm\private\%h copy $(OUTDIR)\atkmm\private\%h "$(PREFIX)\include\atkmm-$(ATKMM_API_VERSION)\atkmm\private\%h"
	@for %h in ($(ATKMM_BUILD_PRIVATE_HEADERS)) do @if exist ..\untracked\atk\atkmm\private\%h copy ..\untracked\atk\atkmm\private\%h "$(PREFIX)\include\atkmm-$(ATKMM_API_VERSION)\atkmm\private\%h"
	@copy ".\atkmm\atkmmconfig.h" "$(PREFIX)\lib\atkmm-$(ATKMM_API_VERSION)\include\"
	@copy ..\codegen\m4\*.m4 $(PREFIX)\lib\atkmm-$(ATKMM_API_VERSION)\proc\m4
