...
install: all
	...
	@for %h in ($(ATKMM_BUILD_PRIVATE_HEADERS)) do @if exist vs$(VSVER)\$(CFG)\$(PLAT)\atkmm\private\%h copy vs$(VSVER)\$(CFG)\$(PLAT)\atkmm\private\%h "$(PREFIX)\include\atkmm-$(ATKMM_MAJOR_VERSION).$(ATKMM_MINOR_VERSION)\atkmm\private\%h"
	@for %h in ($(ATKMM_BUILD_PRIVATE_HEADERS)) do @if exist ..\untracked\atk\atkmm\private\%h copy ..\untracked\atk\atkmm\private\%h "$(PREFIX)\include\atkmm-$(ATKMM_MAJOR_VERSION).$(ATKMM_MINOR_VERSION)\atkmm\private\%h"
